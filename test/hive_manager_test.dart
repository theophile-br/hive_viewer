...
void main() {
  group("HiveManager Class Unit Test", () {
    ...
    test(
      "Should throw an error if you provide path doesn't exist",
      () => {
        expect(
          () => HiveService("not a directory"),
          throwsA(isA<NotADirectory>()),
        ),
      },
    );
    ...
    test("Should throw an error if you provide a file", () {
      final pathToDatabase =
          p.join("test_resources", "hive_data_generator.dart");
    ...
    test("Should throw an error if the folder doesn't contain hive files", () {
      ...
      expect(
        () => HiveService(pathToDatabase),
        throwsA(isA<HiveFileNotFoundInDirectory>()),
      );
    });
    ...
    test("Should open a folder And load box name", () async {
      final pathToDatabase = p.join("test_resources", "hive_data");
      expect(
        () => HiveService(pathToDatabase).getAll("dontexist"),
        throwsA(isA<CollectionNameDoesNotExist>()),
      );
    });
    ...
    test(
        "Should open a hive file and read json stringify data by converting it",
        () async {
      final pathToDatabase = p.join("test_resources", "hive_data");
      final hiveManager = HiveService(pathToDatabase);
      Map<dynamic, dynamic> dogs =
          await hiveManager.getAll("dogsjsonstringify");
