...
class HiveDataTest {
  ...
  static final Map<String, Map<String, Object>> data = {
    ...
    "dogs": {
      "medor": {'name': 'Medor', 'age': 4},
      "youki": {'name': 'Youki', 'age': 2},
    },
    "dogsjsonstringify": {
      "medor": jsonEncode({'name': 'Medor', 'age': 4}),
      "youki": jsonEncode({'name': 'Youki', 'age': 2}),
    },
  };
  ...
  initTestData() async {
    ...
    catsBox.clear();
    data["cats"]!.forEach((key, value) async {
      await catsBox.put(key, value);
    });
    final dogBox = await Hive.openBox("dogs");
    dogBox.clear();
    data["dogs"]!.forEach((key, value) async {
      await dogBox.put(key, value);
    });
    final dogJSONStringifyBox = await Hive.openBox("dogsjsonstringify");
    dogJSONStringifyBox.clear();
    data["dogsjsonstringify"]!.forEach((key, value) async {
      await dogJSONStringifyBox.put(key, value);
    });
  }
}
